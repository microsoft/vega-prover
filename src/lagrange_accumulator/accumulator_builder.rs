...
///   small-value prefix; the suffix loop degenerates to a single iteration
///   with weight `F::ONE`.
/// - **Partial-small (`0 < l0 < ell_b`)**: the first `l0` instance bits go
  ...
  SV: SmallValue + Add<Output = SV> + Sub<Output = SV>,
{
  ...
  assert!(
    l0 > 0 && l0 <= ell_b,
    "build_accumulators_neutronnova requires 0 < l0 <= ell_b. Got l0={}, ell_b={}",
    l0,
    ell_b
  );
...
/// Builds NeutronNova accumulators from preextended prefix workspace values.
///
/// This is the same accumulator construction as
/// [`build_accumulators_neutronnova_from_prefix_workspace`], but the expensive
/// boolean-to-`U_2` prefix extension has already happened in prep. The tables
/// are laid out as `suffix_group -> constraint -> beta_values`, where
/// `beta_indices` maps each stored slot back into the full `U_2^l0` domain.
pub(crate) fn build_accumulators_neutronnova_from_prefix_ext_workspace<F, P>(
  ab_ext_prefixes: &[P],
  beta_indices: &[usize],
  num_constraints: usize,
  suffix_groups: usize,
...
where
  F: PrimeField + DelayedReduction<P> + DelayedReduction<F>,
  P: Sync,
{
  assert!(l0 > 0, "prefix workspace requires l0 > 0");
  let ell_b = rhos.len();
  ...
  assert_eq!(
    ab_ext_prefixes.len(),
    suffix_groups * num_constraints * beta_indices.len(),
    "preextended prefix workspace has unexpected length"
  );
  assert_eq!(
    suffix_groups,
    1usize << (ell_b - l0),
  ...
  struct ProductState<F2, P2>
  where
    F2: PrimeField + DelayedReduction<P2> + DelayedReduction<F2>,
  {
    partial_sums: Vec<<F2 as DelayedReduction<P2>>::Accumulator>,
    acc: LagrangeAccumulators<<F2 as DelayedReduction<F2>>::Accumulator, 2>,
    beta_values: Vec<(usize, F2)>,
  }

  impl<F2, P2> ProductState<F2, P2>
  where
    F2: PrimeField + DelayedReduction<P2> + DelayedReduction<F2>,
  {
    fn new(l0: usize, num_betas: usize) -> Self {
      Self {
        partial_sums: vec![<F2 as DelayedReduction<P2>>::Accumulator::zero(); num_betas],
    ...
    fn reset_partial_sums(&mut self) {
      for sum in &mut self.partial_sums {
        *sum = <F2 as DelayedReduction<P2>>::Accumulator::zero();
      }
  ...
  let (_main_span, main_t) = start_span!("build_accumulators_neutronnova_workspace_main");
  let beta_count = beta_indices.len();
    ...
    for (x_inner, &e_inner_val) in setup.e_inner.iter().enumerate() {
      ...
      for (suffix_idx, &suffix_weight) in setup.suffix_weights.iter().enumerate() {
        ...
        for (slot, &beta_idx) in beta_indices.iter().enumerate() {
          <F as DelayedReduction<P>>::unreduced_multiply_accumulate(
            &mut state.partial_sums[beta_idx],
            &weighted_inner,
            &ab_ext[slot],
          );
...
where
  ...
  SV: SmallValue + Add<Output = SV> + Sub<Output = SV>,
{
  let (_setup_span, setup_t) = start_span!("build_accumulators_neutronnova_preextended_setup");
  let ext_size = 3usize.pow(l0 as u32);
  debug_assert_eq!(
    a_ext_by_constraint.len(),
    b_ext_by_constraint.len(),
    "preextended A/B tables must have identical size"
  );
  debug_assert_eq!(
    a_ext_by_constraint.len() % ext_size,
    0,
    "preextended A/B tables must be laid out as contiguous ext slices"
  );
  ...
  type State<F2, SV2> = SpartanThreadState<F2, SV2, 2>;
  let (_main_span, main_t) = start_span!("build_accumulators_neutronnova_preextended_main");
  let process_outer = |state: &mut State<F, SV>, x_outer: usize| {
    state.reset_partial_sums();
...
mod tests {
  ...
  #[test]
  fn test_preextended_neutronnova_builder_matches_generic() {
    let l0 = 2;
    let n = 1usize << l0;
    let num_cons = 8;
    let left = 4;
    ...
    let e_eq: Vec<Scalar> = (0..(left + right))
      ...
      .collect();
    let rhos = vec![Scalar::from(7u64), Scalar::from(11u64)];

    let a_layers: Vec<Vec<i32>> = (0..n)
      .map(|layer| {
        (0..num_cons)
          .map(|idx| ((layer as i32 * 13 + idx as i32 * 5) % 17) - 8)
          .collect()
      })
      .collect();
    let b_layers: Vec<Vec<i32>> = (0..n)
      .map(|layer| {
        (0..num_cons)
          .map(|idx| ((layer as i32 * 7 + idx as i32 * 9) % 19) - 9)
          .collect()
      })
      .collect();
    ...
    let bit_rev = bit_rev_prefix_table(l0);
    let ext_size = 3usize.pow(l0 as u32);
    let mut a_ext_flat = Vec::with_capacity(num_cons * ext_size);
    let mut b_ext_flat = Vec::with_capacity(num_cons * ext_size);
    let mut a_prefix = vec![0i32; n];
    let mut b_prefix = vec![0i32; n];
    let mut a_buf = Vec::new();
    let mut a_scratch = Vec::new();
    let mut b_buf = Vec::new();
    let mut b_scratch = Vec::new();

    for idx in 0..num_cons {
      ...
      assert_eq!(a_size, ext_size);
      assert_eq!(b_size, ext_size);
      a_ext_flat.extend_from_slice(&a_buf[..a_size]);
      b_ext_flat.extend_from_slice(&b_buf[..b_size]);
    }

    let generic =
      build_accumulators_neutronnova(&a_layers, &b_layers, &e_eq, left, right, &rhos, l0);
    let preextended = build_accumulators_neutronnova_preextended(
      &a_ext_flat,
      &b_ext_flat,
      &e_eq,
      left,

...
After setup inputs, circuits, public values, and randomness are fixed, the online prover is deterministic: it builds the same split instances, derives the same Fiat--Shamir challenges, runs the same folding and sum-check reductions, and assembles the same proof fields. Fresh randomness is the only honest-prover divergence between two proofs of the same statement. It enters through prepared-state rerandomization in [Preparation](./prep.md), the fresh relaxed mask `random_U`, and the zero-knowledge IPA opening.

This matches the byte-equivalence contract in [Specification scope](../spec/scope.md): an independent prover driven with identical inputs and identical randomness produces identical proof bytes. Different honest proofs of the same statement differ only because the rerandomization and zero-knowledge openings consume fresh randomness.
